...
pub enum sgx_status_t {
	...
	SGX_ERROR_OUT_OF_MEMORY = 0x0000_0003,     /* Not enough memory is available to complete
	                                            * this operation */
	SGX_ERROR_ENCLAVE_LOST = 0x0000_0004, /* Enclave lost after power transition or used in
	...
	SGX_ERROR_ECALL_NOT_ALLOWED = 0x0000_1007, /* The ECALL is not allowed at this time, e.g.
	                                           * ecall is blocked by the dynamic entry table,
	                                           * or nested ecall is not allowed during
	                                           * initialization */
	SGX_ERROR_OCALL_NOT_ALLOWED = 0x0000_1008, /* The OCALL is not allowed at this time, e.g.
	                                            * ocall is not allowed during exception
	                                            * handling */
	...
	SGX_ERROR_NDEBUG_ENCLAVE = 0x0000_2004,   /* The enclave is signed as product enclave, and
	...
	SGX_ERROR_INVALID_VERSION = 0x0000_200D, /* Metadata version is inconsistent between uRTS and
	                                     * sgx_sign or uRTS is incompatible with current
	                                     * platform. */
	SGX_ERROR_MODE_INCOMPATIBLE = 0x0000_200E, /* The target enclave 32/64 bit mode or sim/hw
	                                            * mode is incompatible with the mode of current
	                                            * uRTS. */
	SGX_ERROR_ENCLAVE_FILE_ACCESS = 0x0000_200F, /* Can't open enclave file. */
	SGX_ERROR_INVALID_MISC = 0x0000_2010,        /* The MiscSelct/MiscMask settings are not
	                                              * correct. */
	SGX_ERROR_INVALID_LAUNCH_TOKEN = 0x0000_2011, /* The launch token is not correct. */

	SGX_ERROR_MAC_MISMATCH = 0x0000_3001, /* Indicates verification error for reports, sealed
	                                       * datas, etc */
	SGX_ERROR_INVALID_ATTRIBUTE = 0x0000_3002, /* The enclave is not authorized, e.g.,
	                                            * requesting invalid attribute or launch key
	                                            * access on legacy SGX platform without FLC. */
	SGX_ERROR_INVALID_CPUSVN = 0x0000_3003, /* The cpu svn is beyond platform's cpu svn value */
	SGX_ERROR_INVALID_ISVSVN = 0x0000_3004, /* The isv svn is greater than the enclave's isv
	                                         * svn */
	SGX_ERROR_INVALID_KEYNAME = 0x0000_3005, /* The key name is an unsupported value */

	SGX_ERROR_SERVICE_UNAVAILABLE = 0x0000_4001, /* Indicates aesm didn't respond or the
	...
	SGX_ERROR_SERVICE_INVALID_PRIVILEGE = 0x0000_4004, /* Enclave not authorized to run, .e.g.
	                                          * provisioning enclave hosted in an app without
	                                          * access rights to /dev/sgx_provision */
	...
	SGX_ERROR_NETWORK_FAILURE = 0x0000_4007,     /* Network connecting or proxy setting issue
	                                              * is encountered */
	SGX_ERROR_AE_SESSION_INVALID = 0x0000_4008, /* Session is invalid or ended by server */
	SGX_ERROR_BUSY = 0x0000_400A,               /* The requested service is temporarily not
	                                             * availabe */
	SGX_ERROR_MC_NOT_FOUND = 0x0000_400C, /* The Monotonic Counter doesn't exist or has been
	                                       * invalided */
	SGX_ERROR_MC_NO_ACCESS_RIGHT = 0x0000_400D, /* Caller doesn't have the access right to
	                                             * specified VMC */
	...
	SGX_ERROR_KDF_MISMATCH = 0x0000_4011, /* Key derivation function doesn't match during key
	                                     * exchange */
	SGX_ERROR_UNRECOGNIZED_PLATFORM = 0x0000_4012, /* EPID Provisioning failed due to platform
	                                                * not recognized by backend server */
	SGX_ERROR_UNSUPPORTED_CONFIG = 0x0000_4013, /* The config for trigging EPID Provisiong or
	                                             * PSE Provisiong&LTP is invalid */
	...
	SGX_ERROR_PCL_NOT_ENCRYPTED = 0x0000_6002, /* trying to load a plain enclave using
	                                        * sgx_create_encrypted_enclave */
	SGX_ERROR_PCL_MAC_MISMATCH = 0x0000_6003, /* section mac result does not match build time
	                                           * mac */
	SGX_ERROR_PCL_SHA_MISMATCH = 0x0000_6004, /* Unsealed key MAC does not match MAC of key
	                                           * hardcoded in enclave binary */
	SGX_ERROR_PCL_GUID_MISMATCH = 0x0000_6005, /* GUID in sealed blob does not match GUID
	                                            * hardcoded in enclave binary */

	/* SGX errors are only used in the file API when there is no appropriate EXXX (EINVAL, EIO
	 * etc.) error code */
	SGX_ERROR_FILE_BAD_STATUS = 0x0000_7001, /* The file is in bad status, run sgx_clearerr to
	                                          * try and fix it */
	SGX_ERROR_FILE_NO_KEY_ID = 0x0000_7002, /* The Key ID field is all zeros, can't re-generate
	                                         * the encryption key */
	SGX_ERROR_FILE_NAME_MISMATCH = 0x0000_7003, /* The current file name is different then the
	                                             * original file name (not allowed,
	                                             * substitution attack) */
	SGX_ERROR_FILE_NOT_SGX_FILE = 0x0000_7004, /* The file is not an SGX file */
	SGX_ERROR_FILE_CANT_OPEN_RECOVERY_FILE = 0x0000_7005, /* A recovery file can't be opened, so flush
	                                            * operation can't continue (only used when no
	                                            * EXXX is returned) */
	SGX_ERROR_FILE_CANT_WRITE_RECOVERY_FILE = 0x0000_7006, /* A recovery file can't be written,
	                                                        * so flush operation can't continue
	                                                        * (only used when no EXXX is
	                                                        * returned) */
	SGX_ERROR_FILE_RECOVERY_NEEDED = 0x0000_7007, /* When openeing the file, recovery is
	                                               * needed, but the recovery process failed */
	SGX_ERROR_FILE_FLUSH_FAILED = 0x0000_7008, /* fflush operation (to disk) failed (only used
	                                            * when no EXXX is returned) */
	SGX_ERROR_FILE_CLOSE_FAILED = 0x0000_7009, /* fclose operation (to disk) failed (only used
	                                            * when no EXXX is returned) */

	SGX_ERROR_UNSUPPORTED_ATT_KEY_ID = 0x0000_8001, /* platform quoting infrastructure does not
	                                                 * support the key. */
	SGX_ERROR_ATT_KEY_CERTIFICATION_FAILURE = 0x0000_8002, /* Failed to generate and certify
	                                                        * the attestation key. */
	SGX_ERROR_ATT_KEY_UNINITIALIZED = 0x0000_8003, /* The platform quoting infrastructure does
	                                                * not have the attestation key available to
	                                                * generate quote. */
	SGX_ERROR_INVALID_ATT_KEY_CERT_DATA = 0x0000_8004, /* TThe data returned by the platform
	                                                    * library's sgx_get_quote_config() is
	                                                    * invalid. */
	SGX_ERROR_PLATFORM_CERT_UNAVAILABLE = 0x0000_8005, /* The PCK Cert for the platform is not
	                                                    * available. */

	SGX_INTERNAL_ERROR_ENCLAVE_CREATE_INTERRUPTED = 0x0000_F001, /* The ioctl for
	                                                              * enclave_create unexpectedly
	                                                              * failed with EINTR. */
